...
export class ZhiXueAdapter extends EventEmitter {
    ...
    async captureWithHtml2Canvas(element) {
        try {
            // 先尝试加载本地打包的 html2canvas，再回退 CDN
            const loadHtml2Canvas = () => new Promise((resolve, reject) => {
                ...
                const tryLoad = (src) => {
                    ...
                    s.src = src;
                    s.async = true;
                    s.onload = () => window.html2canvas ? resolve() : reject(new Error(`html2canvas未初始化: ${src}`));
                ...
                try {
                    const localUrl = typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.getURL
                        ? chrome.runtime.getURL('libs/html2canvas.min.js')
                        : null;
                    if (localUrl) {
                        tryLoad(localUrl);
                    } else {
                        reject(new Error('未提供本地html2canvas'));
                    }
                } catch {
                    reject(new Error('加载本地html2canvas失败'));
                }
            });
